...
def check_image_intent(text):
    ...
    triggers = [
        "generate image", "create image", "make image", "generate photo",
        "create photo", "make picture", "draw", "photo of", "image of",
        "picture of", "फोटो बनाओ", "इमेज बनाओ", "चित्र बनाओ", "તસવીર બનાવો"
    ]
    for tr in triggers:
        if tr in text_lower:
            prompt = re.sub(rf".*?{tr}\s*(of|ke liye|ki|ka)?\s*", "", text_lower, count=1).strip()
            return True, (prompt if prompt else text_lower)
    return False, ""
...
for idx, msg in enumerate(st.session_state.messages):
    with st.chat_message(msg["role"]):
        if msg.get("is_image", False):
            st.image(msg["content"], caption="Generated by BC Tech AI", use_container_width=True)
        else:
...
if query:
    ...
    is_img, img_prompt = check_image_intent(query)
